...
import com.picker.filter.loader.AudioLoader;
import com.picker.filter.loader.FileLoader;
import com.picker.filter.loader.ImageLoader;
import com.picker.filter.loader.VideoLoader;
...
public class FileLoaderCallbacks implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    public FileLoaderCallbacks(Context context, FilterResultCallback resultCallback, int type, String[] suffixArgs) {
        ...
        if (suffixArgs != null && suffixArgs.length > 0) {
            ...
            Log.e("FileLoaderCallbacks", "mSuffixRegex = " + mSuffixRegex);
        }
    }
    ...
    private void onFileResult(Cursor data) {
        ...
        Log.e("FileLoaderCallbacks", "onFileResult count = " + data.getCount());
        if (data.getPosition() != -1) {
            data.moveToPosition(-1);
        }
        while (data.moveToNext()) {
